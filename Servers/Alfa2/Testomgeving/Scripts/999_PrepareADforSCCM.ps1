...
# 3) Nu moeten we in ADSIedit (Waar al de instellingen van ADDS staan) een System Management container maken onder de "System"
# 3.1) Verbind met ADSIedit:
...
# 3.2) Maak de System Management container aan in ADSIedit:
...
# 4) Nu moeten we de permissies (ALLE PERMISSIES!) van deze System Management container delegeren aan de Papa2 server
# Belangrijk hier is dat je ook de instelling "Applies to this object and all descendant objects" moet selecteren
# 4.1) Geef ALLE permissies van deze container aan de Papa2 server:
...
# 4.2) Get Papa2 als computer object:
$SCCMserver = get-adcomputer "Papa2"

# 4.3) Get identity reference van het computer object:
...
# 4.4) Stel permissions in als ALLOW full control (full control = GenericAll hieronder):
...
# 4.5) Dit zal de gekozen permissies toepassen op de System Management container:
...
# 5) Extend Active Directory Schema:
...
# 5.1) Eerst de folder "ExtendADSchema" uit de VB share folder lokaal kopiëren naar Alfa2 server:
...
# 5.2) Dan voer je het script (ALS ADMINISTRATOR!) uit om de AD schema te extenden:
